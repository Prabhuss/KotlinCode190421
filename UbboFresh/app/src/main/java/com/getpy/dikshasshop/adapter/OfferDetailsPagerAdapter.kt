...
class OfferDetailsPagerAdapter(val context: Context, val images: ArrayList<GetOfferDetailsData>) : PagerAdapter() {
    ...
    override fun instantiateItem(container: ViewGroup, position: Int): Any {
        ...
        Glide.with(context).load(model.OfferImgURL).apply(options).into(binding.imageView)*/
        binding.imageView.load(model.OfferImgURL) {
            placeholder(R.drawable.ic_no_image_found)
            error(R.drawable.ic_no_image_found)
        }
